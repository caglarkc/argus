...
import pytest
import textwrap
from argus.config.manager import ConfigManager, ConfigError
from argus.config.config import AppConfig
...
def create_config_file(tmp_path: Path, content: str) -> Path:
    ...
    """
    cfg = tmp_path / "argus_config.yaml"
...
def test_resolve_without_args_uses_default_agent(tmp_path: Path):
    ...
    assert mgr.get_active_agent_name() == "argus"
    assert mgr.get_active_model_name() == "argus-model"
...
def test_list_agent_names(tmp_path: Path):
    ...
    assert mgr.list_agent_names() == ["argus", "codex"]
...
def test_cli_overrides_model_and_keys(tmp_path: Path):
    ...
    args = SimpleNamespace(agent="argus",
        model="cli-model",
        api_key="cli-key",
        base_url="https://cli/",
        temperature=0.8,
...
def test_env_overrides_when_yaml_missing(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("ARGUS_ARGUS_MODEL", "env-model")
    monkeypatch.setenv("ARGUS_ARGUS_API_KEY", "env-key")
    monkeypatch.setenv("ARGUS_ARGUS_BASE_URL", "https://env")
