...
from argus.llm.llm_basics import ToolCall, LLMMessage
...
from argus.memory.memory_monitor import MemoryMonitor
...
class History:
    ...
    def add_message(self, role: MessageRole, content: Any) -> None:
        if role == "system":
            raise ValueError("system message 0 message；message replace_system().")
        self._items.append({"type": "message", "role": role, "content": content})

    def add_item(self, item: Any) -> None:
        """messagePydanticmessage,message"""
    ...
    def to_llm_messages(self) -> List[LLMMessage]:
        llm_messages = []
        for msg in self._items[1:]:  #messagesystem
            ...
            if msg.get("type") in {"function_call", "custom_tool_call", "function"}:
                ...
                content = ""
                tool_call = ToolCall(call_id = msg.get("call_id", ""),
                    name = msg.get("name", ""),
                    arguments = arguments,
                    type = msg.get("type"),)
...
class CodexAgent(BaseAgent):
    ...
    async def run(self, user_message: str) -> AsyncGenerator[AgentEvent, None]:
        ...
        self.trajectory_recorder.start_recording(task=user_message, provider=provider, model=model_name, max_steps=self.turn_cnt_max)
    ...
    def record_turn_messages(self, messages: List[Dict[str, Any]], responses) -> None:
        """message"""
        #1. messagemessagesmessage, pydantic -> dict
        converted_messages = []
        llm_msg = None
        model_name = self.config_mgr.get_active_model_name() or "gpt-5-codex"
        for msg in messages:
        ...
        if isinstance(responses, BaseModel):
            ...
            for out in responses.output:
                if out.type == "function_call" or out.type == "custom_tool_call":
                    tool_call = ToolCall(call_id = out.call_id,
                        name = out.name,
                        arguments = json.loads(out.arguments) if out.type == "function_call" else out.input,
                        type = out.type,)
                    tool_calls.append(tool_call)
 
            resp = LLMResponse(content = responses.output_text,
                    tool_calls = tool_calls,
                    usage = responses.usage,
                    model = model_name,
                    finish_reason = "completed",)

        self.trajectory_recorder.record_llm_interaction(messages = converted_messages,
                response = resp,
    ...
    async def _responses_event_process(self, messages:List[HistoryItem], params) -> AsyncGenerator[AgentEvent, None]:
        """messageLLMmessage,messageagentmessage"""
