...
from.argus.argus_agent import ArgusAgent
...
class AgentManager:
    ...
    async def create_sub_agent(self, name: str) -> BaseAgent:
        ...
        if not self.is_supported(normalized):
            raise ValueError(f"Agent '{name}' is not declared in configuration. "
    ...
    async def _create_agent(self, normalized_name: str) -> BaseAgent:
        if normalized_name == "argus":
            return ArgusAgent(self._config_mgr, self._cli, self._tool_mgr)
