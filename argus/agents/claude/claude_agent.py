...
from argus.llm.llm_basics import LLMResponse, LLMMessage, ToolCall, ToolCallResult
from argus.llm.llm_events import LLM_Events
from argus.utils.trajectory_recorder import TrajectoryRecorder
...
from argus.agents.agent_events import AgentEvent, Agent_Events
from argus.agents.claude.system_reminder import (generate_system_reminders, emit_reminder_event, reset_reminder_session,
...
class ClaudeAgent(BaseAgent):
    ...
    async def run(self, user_message: str) -> AsyncGenerator[AgentEvent, None]:
        try:
            ...
            # quotamessage,message,message,message
            """
    ...
    def _build_claude_messages(self) -> List[LLMMessage]:
        ...
        dynamic_reminders = generate_system_reminders(has_context=has_context,
                agent_id=self.type,
    ...
    async def _check_quota(self) -> bool:
        try:
            ...
            self.trajectory_recorder.record_llm_interaction(messages=[LLMMessage(role="user", content="quota")],
    ...
    async def _detect_new_topic(self, user_input: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            topic_llm_response = LLMResponse(content=content,
                    model= model_name,
                    finish_reason="stop",
                    usage=None,
                    tool_calls=[])

            self.trajectory_recorder.record_llm_interaction(messages=[
    ...
    async def _query_recursive(self, messages: List[LLMMessage], depth: int = 0) -> AsyncGenerator[AgentEvent, None]:
        try:
            ...
            if assistant_message:
                ...
                llm_response = LLMResponse(content=assistant_message.content or "",
                ...
                self.trajectory_recorder.record_llm_interaction(messages=messages,
            ...
            for tool_call in tool_calls:
                yield AgentEvent.tool_call(call_id=tool_call.get("id", "unknown"),
                        name=tool_call["name"],
            ...
            #message
            has_context = bool(self.context and len(self.conversation_history) > 1)
            new_dynamic_reminders = generate_system_reminders(has_context=has_context,
                    agent_id=self.type,
                    todo_items=self.todo_items)

            for reminder in new_dynamic_reminders:
                reminder_message = LLMMessage(role="user",
        ...
        try:
            ...
            async for event in self.llm_client.astream_response(formatted_messages, **params):
                if event.type == LLM_Events.ASSISTANT_DELTA:
                    ...
                    tc = ToolCall(call_id= tool_call.get("call_id") if tool_call else "unknown",
    ...
    async def _execute_tools(self, tool_calls: List[Dict[str, Any]]) -> AsyncGenerator[tuple[AgentEvent, Optional[LLMMessage]], None]:
        ...
        for tool_call in tool_calls:
            try:
                ...
                event = AgentEvent.tool_result(call_id=tool_call.get("id", "unknown"),
                        name=tool_call["name"],
                ...
                error_message = LLMMessage(role="tool",
    ...
    async def _execute_single_tool_with_result(self, tool_call: Dict[str, Any],) -> tuple[ToolCallResult, LLMMessage]:
        try:
            tool_call_obj = ToolCall(call_id=tool_call.get("id", "unknown"),
                    name=tool_call["name"],
            ...
            tc_res = ToolCallResult(call_id=tool_call.get("id", "unknown"),
                    result=result,
            ...
            error_msg = f"Error executing tool '{tool_call['name']}': {str(e)}"
            error_result = ToolCallResult(call_id=tool_call.get("id", "unknown"),
                    error=error_msg,)
            error_message = LLMMessage(role="tool",
                    content=f"Error: {error_msg}",
                    tool_call_id=tool_call.get("id", "unknown"))
