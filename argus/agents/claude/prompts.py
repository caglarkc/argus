...
- Identify potential issues or improvement opportunities"""
    ...
    def build_context(project_path: str) -> Dict:
        ...
        try:
            ...
            claude_md_path = os.path.join(project_path, 'ARGUS.md')
            if os.path.exists(claude_md_path):
                with open(claude_md_path, 'r', encoding='utf-8') as f:
                    context['argus_md'] = f.read()
    ...
    def _get_file_tree(project_path: str, max_depth: int = 3) -> str:
        ...
        try:
            result = subprocess.run(['tree', '-L', str(max_depth), '-I', '__pycache__|*.pyc|.git|node_modules'],
                cwd=project_path,
                capture_output=True,
                text=True,
                timeout=10)
            ...
            for item in sorted(os.listdir(path)):
                ...
                if os.path.isdir(item_path):
                    ...
                    if current_depth < max_depth - 1:
                        subtree = ClaudeCodePrompts._manual_file_tree(item_path, max_depth, current_depth + 1)
                        if subtree:
