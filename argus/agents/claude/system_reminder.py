...
class SystemReminderService:
    ...
    def __init__(self):
        self.session_state = SessionReminderState(session_start_time=time.time())
...
NEVER proactively create documentation files (*.md) or README files. Only create documentation files if explicitly requested by the User.
    ...
    # def get_system_reminder_end(#     self, 
    ...
    def generate_reminders(self, 
        ...
        agent_id: Optional[str] = None,
        todo_items: Optional[List[Dict]] = None) -> List[ReminderMessage]:
        """
        ...
        # message:messageget_system_reminder_end,message_dispatch_todo_eventmessage
                
        return reminders

    def _dispatch_todo_event(self, 
        agent_id: Optional[str] = None,
        todo_items: Optional[List[Dict]] = None) -> Optional[ReminderMessage]:
        ...
        if not todo_items or len(todo_items) == 0:
            ...
            if reminder_key not in self.session_state.reminders_sent:
                ...
                return self._create_reminder_message('todo_empty',
        ...
        if todo_items and len(todo_items) > 0:
            ...
            if reminder_key not in self.session_state.reminders_sent:
                ...
                reminder = self._create_reminder_message('todo_updated',
                    ReminderCategory.TASK,
                    ReminderPriority.MEDIUM,
    ...
    def _dispatch_security_event(self) -> Optional[ReminderMessage]:
        ...
        if (self.session_state.last_file_access > 0 and 
            ...
            return self._create_reminder_message('security',
    ...
    def _dispatch_performance_event(self) -> Optional[ReminderMessage]:
        ...
        if (session_duration > 30 * 60 and 
            ...
            return self._create_reminder_message('performance',
    ...
    def generate_file_change_reminder(self, context: Dict[str, Any]) -> Optional[ReminderMessage]:
        ...
        return self._create_reminder_message('file_changed',
    ...
    def _create_reminder_message(self,
        ...
        content: str,
        timestamp: float) -> ReminderMessage:
        """Create a formatted reminder message"""
        return ReminderMessage(role='user',
...
def generate_system_reminders(has_context: bool = False,
    agent_id: Optional[str] = None,
    todo_items: Optional[List[Dict]] = None) -> List[ReminderMessage]:
    """
...
def emit_tool_execution_event(tool_call, agent_type: str, todo_items: List[Dict] = None) -> Optional[List]:
    ...
    Args:
        tool_call: message,message name message arguments
        agent_type: Agent message
        todo_items: message TODO message(message todo_write message)
    
    Returns:
        message todo_write message,message todo_items,message None
    """
    from argus.llm.llm_basics import ToolCall
    current_time = time.time()
    ...
    # TODO message
        ...
        return new_todos  # message TODO message Agent message
...
def determine_todo_change_type(previous_todos: List, new_todos: List) -> str:
    ...
    Args:
        previous_todos: message TODO message
        new_todos: message TODO message
    
    Returns:
        message:'added', 'removed', message 'modified'
    """
