...
from argus.tools.base_tool import ToolRiskLevel
from argus.utils.permission_manager import PermissionLevel, PermissionManager
from argus.cli.highlighted_content import create_enhanced_tool_result_display, HighlightedContentDisplay
from argus.agents.agent_events import Agent_Events, AgentEvent
...
class CLIConsole:
    ...
    def __init__(self, perm_mgr: Optional[PermissionManager] = None):
        ...
        self.approval = ApprovalService(permission_manager= perm_mgr or PermissionManager(PermissionLevel.LOCKED), 
    ...
    def show_status_bar(self, model_name:str = "qwen3-coder-plus", permission_level:str = "yolo"):
        self.status_bar.show(model_name=model_name,
...
class Printer:
    """message(message,message rich message)"""
...
class StatusBar:
    ...
    def show(self, *, model_name: str,
             permission_level: str | None = None, # message:locked/edit_only/planning/yolo message None
...
class ApprovalService:
    ...
    async def confirm(self, tool_name, tool_args, tool=None) -> bool:
        ...
        while True:
            try:
                resp = await session.prompt_async(HTML('<ansiblue><b>Allow this tool execution? (y/n/a for always): </b></ansiblue>'))
