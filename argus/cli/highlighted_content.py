...
class HighlightedContentDisplay:
    ...
    @staticmethod
    def create_edit_result_display(old_content: str,
        new_content: str,
        old_text: str,
        new_text: str,
        file_path: str) -> Panel:
    ...
    @staticmethod
    def create_write_file_result_display(content: str,
        file_path: str,
        is_new_file: bool,
        old_content: Optional[str] = None) -> Panel:
        ...
        if is_new_file:
            ...
            return HighlightedContentDisplay.create_highlighted_file_content(content, file_path, added_lines=added_lines)
        else:
            ...
            if old_content:
                ...
                return HighlightedContentDisplay.create_highlighted_file_content(content, file_path,
    ...
    @staticmethod
    def create_side_by_side_comparison(old_content: str,
        new_content: str,
        file_path: str,
        max_lines: int = 30) -> Panel:
        ...
        if len(old_lines) > max_lines or len(new_lines) > max_lines:
            table.add_row(f"... ({len(old_lines) - max_lines} more)" if len(old_lines) > max_lines else "",
...
def create_enhanced_tool_result_display(result_data: dict, tool_name: str) -> Panel:
    ...
    if operation == 'edit_file':
        ...
        return HighlightedContentDisplay.create_edit_result_display(old_content, new_content, old_text, new_text, file_path)
        ...
        return HighlightedContentDisplay.create_write_file_result_display(content, file_path, is_new_file, old_content)
    
    else:
        # Fallback for other operations
        return Panel(Text(summary),
            title=f"✓ {tool_name}",
            title_align="left",
            border_style="green",
            padding=(0, 1))
