...
from argus.agents.agent_events import Agent_Events
from argus.agents.agent_manager import AgentManager
from argus.cli.command_processor import CommandProcessor
from argus.config.manager import ConfigManager
from argus.utils.key_binding import create_key_bindings
...
class PromptDriver:
    def __init__(self, 
                 ...
                 exit_sentinel: str = "__ARGUS_QUIT__",) -> None:
        self._cli:CLIConsole = cli
        self.exit_sentinel = exit_sentinel
        kbs = create_key_bindings(console_getter=lambda: cli,
...
class EventPump:
    """message agent message → CLI message"""
...
class TurnRunner:
    ...
    async def run_once(self, *, user_input: str, session_id: str, cancel_token: CancellationToken) -> RunOutcome:
        try:
            # message Hook
            ok, msg, _ = await self.hook_mgr.emit(HookEvent.UserPromptSubmit,
            ...
            # message Hook
            ok2, msg2, extra = await self.hook_mgr.emit(HookEvent.Stop,
                base_payload={"session_id": session_id, "prompt": user_input},)
            if msg2:
            ...
            if extra.get("additionalContext") and self.agent_mgr.current:
                self.agent_mgr.current.conversation_history.append(LLMMessage(role="user", content=extra["additionalContext"]))
...
class HeadlessRunner:
    ...
    async def run(self, *, prompt: str, session_id: str, set_yolo: bool = True) -> RunOutcome:
        ...
        outcome = await self._runner.run_once(user_input=prompt, session_id=session_id, cancel_token=cancle)
        return outcome
...
class InteractiveSession:
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            outcome = await self._runner.run_once(user_input=line, session_id=sid, cancel_token=self.cancel_event)
