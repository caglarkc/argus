...
def _parse_one_hunk(lines: List[str], line_number: int) -> Tuple[Hunk, int]:
    ...
    if first_line.startswith(UPDATE_FILE_MARKER):
        ...
        while remaining:
            ...
            chunk, chunk_lines = _parse_update_file_chunk(remaining, line_number + parsed_lines, allow_missing_context=(len(chunks) == 0))
            chunks.append(chunk)
    ...
    raise InvalidHunkError(f"'{first_line}' is not a valid hunk header. Valid hunk headers: '*** Add File: {{path}}', '*** Delete File: {{path}}', '*** Update File: {{path}}'",
        line_number,)
...
def _parse_update_file_chunk(lines: List[str], line_number: int, allow_missing_context: bool) -> Tuple[UpdateFileChunk, int]:
    ...
    if lines[0] == EMPTY_CHANGE_CONTEXT_MARKER:
        ...
        if not allow_missing_context:
            raise InvalidHunkError(f"Expected update hunk to start with a @@ context marker, got: '{lines[0]}'",
                line_number,)
    ...
    for line in lines[start_index:]:
        ...
        if line == "":
            ...
            if first == ' ':
                ...
                if parsed_lines == 0:
                    raise InvalidHunkError(f"Unexpected line found in update hunk: '{line}'. Every line should start with ' ' (context line), '+' (added line), or '-' (removed line)",
                        line_number + 1,)
                break
        parsed_lines += 1
...
def _compute_replacements(original_lines: List[str],
    file_path: Path,
...
def _apply_replacements(original_lines: List[str], reps: List[Tuple[int, int, List[str]]]) -> List[str]:
    ...
    for start_idx, old_len, new_lines in reps:
        if start_idx < cursor:
            start_idx = cursor  # best effort message
