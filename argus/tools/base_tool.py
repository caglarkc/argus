...
from argus.llm.llm_basics import ToolCallConfirmationDetails, ToolCallResult

class ToolRiskLevel(Enum):
    """Tool risk levels for permission control."""
    SAFE = "safe"           # message,message
    LOW = "low"             # message,message
    MEDIUM = "medium"       # message,message
    HIGH = "high"           # message,message
...
class BaseTool(ABC):
    ...
    async def get_confirmation_details(self, **kwargs) -> Optional[ToolCallConfirmationDetails]:
        ...
        return ToolCallConfirmationDetails(type="exec",
