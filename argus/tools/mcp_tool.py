...
from argus.tools.base_tool import BaseTool, ToolRiskLevel
from argus.llm.llm_basics import ToolCallResult, ToolCallResultDisplay
from argus.config.manager import ConfigManager
from argus.tools.tool_manager import register_instance 

def _make_tool_result(call_id: str,
    message: str,
    *,
    is_error: bool = False,
    summary: Optional[str] = None,
    metadata: Optional[Dict[str, Any]] = None,
    display_markdown: Optional[str] = None,) -> ToolCallResult:
    display = ToolCallResultDisplay(markdown=display_markdown if display_markdown is not None else message,
        summary=summary or "")
    return ToolCallResult(call_id=call_id,
...
class MCPServerManager:
    """
    - message/message MCP stdio server
    - message list_tools / call_tool
    ...
    async def add_stdio_server(self, name: str, command: str, args: Iterable[str], *, timeout: float = 30.0) -> None:
        """
        message stdio message MCP server.
        ...
        if not _is_executable(command):
            raise MCPServerLaunchError(f"Command '{command}' not found or not executable. Please install or fix PATH.")
        ...
        async with lock:
            ...
            task = asyncio.create_task(self._owner_task(name, command, list(args), ready_evt, stop_evt),
            ...
            try:
                ...
                raise MCPServerLaunchError(f"Timed out waiting for MCP server '{name}' to become ready (timeout={timeout}s).")
            ...
            if task.done() and task.exception():
                ...
                self._stops.pop(name, None)
                self._ready.pop(name, None)
                raise MCPServerLaunchError(f"Failed to start MCP server '{name}': {ex}")

    async def _owner_task(self,
        name: str,
        command: str,
        args: List[str],
        ready_evt: asyncio.Event,
...
class MCPRemoteTool(BaseTool):
    ...
    - execute() message server message tool
    - build() message LLM message/message(message provider message)
    """
    def __init__(self,
        *,
        server: str,
        ...
        providers: Optional[Iterable[str]] = None,  # message) -> None:
        # BaseTool message
        self.name = spec.name
    ...
    async def execute(self, **kwargs) -> ToolCallResult:
        ...
        try:
            ...
            return _make_tool_result(call_id=call_id,
        ...
        for item in content:
            ...
            if t == "text":
                ...
                if self._save_images_dir and b64:
                    ...
                    path = os.path.join(self._save_images_dir,
    ...
    def build(self, provider: str = "", func_type: str = "") -> Mapping[str, Any]:
        res = {}
        if provider.lower() in {"argus", "codex", "openai"}:
            res = {
                "type": "function",
                "function": {
                    "name": self.name,
...
def _match_includes(name: str, patterns: Optional[List[str]]) -> bool:
    """include: ["browser_*", "files.read", "*"] -> message.None/message."""
    if not patterns:
        return True
...
async def sync_mcp_servers(*, cfg_mgr: ConfigManager, overwrite: bool = True,) -> Tuple[MCPServerManager, List[str]]:
    """
    message config.mcp:
      - message enabled message MCP servers
      - message
      - message include message
      - message tool message:
      (MCPServerManager, message)
    """
    ...
    for srv in servers:
        ...
        # 1. message server, message stdio
        if srv.type == "http":
        ...
        for t in tools:
            ...
            local_tool = MCPRemoteTool(server=name,
                manager=mcp_mgr,
                spec=_RemoteSpec(name=tool_name,
                    description=description,
                    input_schema=input_schema,),
                save_images_dir=save_dir,
                risk_level=ToolRiskLevel.MEDIUM,)

            register_instance(name=local_tool.name,
                instance=local_tool,
                overwrite=overwrite,)
