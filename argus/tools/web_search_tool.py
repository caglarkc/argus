...
@register_tool(name="web_search", providers=["claude", "argus"])
class WebSearchTool(BaseTool):
    ...
    async def execute(self, **kwargs) -> ToolCallResult:
        ...
        if validation_error:
            return ToolCallResult(call_id=kwargs.get("call_id", ""),
    ...
    def _parse_search_results(self, data: Dict[str, Any]) -> List[SearchResult]:
        ...
        for i, result in enumerate(organic_results):
            search_result = SearchResult(title=result.get("title", "No Title"),
        ...
        if knowledge_graph:
            kg_result = SearchResult(title=f"Knowledge Graph: {knowledge_graph.get('title', 'Information')}",
        ...
        if answer_box:
            answer_result = SearchResult(title=f"Answer: {answer_box.get('title', 'Direct Answer')}",
