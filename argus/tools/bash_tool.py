...
from argus.tools.tool_manager import register_tool

# message
MAX_OUTPUT_LENGTH = 30000
DEFAULT_TIMEOUT = 120  # message(message)
INITIAL_WAIT_TIME = 5.0  # message(message),message
READ_BUFFER_SIZE = 4096  # message(message)
READ_TIMEOUT = 0.5  # message(message)
BACKGROUND_STARTUP_DELAY = 1.0  # message(message)
BACKGROUND_READ_TIMEOUT = 1.0  # message(message)
...
@register_tool(name="bash", providers=["argus", "claude",])
class BashTool(BaseTool):
    ...
    def _prepare_command(self, command: str) -> Tuple[str, Optional[str]]:
        """
        message,message (shell_command, temp_script_path)

        message,message,message.
        message,message bash -c message.
        """
        if os.name == "nt":
        ...
        if '\n' in command:
            try:
                fd, temp_script = tempfile.mkstemp(suffix='.sh', prefix='argus_bash_', text=True)
                try:
    ...
    async def _execute_foreground(self,
        shell_command: str,
        cwd: str,
        timeout: float) -> ToolCallResult:
        """message"""
        try:
            env = os.environ.copy()
            env.update({
    ...
    async def _read_with_progress(self,
        ...
        try:
            done, pending = await asyncio.wait([asyncio.create_task(process.wait())],
    ...
    async def _execute_background(self, shell_command: str, original_command: str, cwd: str) -> ToolCallResult:
        ...
        try:
            ...
            if process.stdout:
                try:
                    stdout_data = await asyncio.wait_for(process.stdout.read(READ_BUFFER_SIZE),
