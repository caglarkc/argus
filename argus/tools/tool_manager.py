...
from argus.utils.permission_manager import PermissionManager 
...
def register_instance(*,
    ...
    """
    message BaseTool.
    - name: message(LLMmessage)
    - providers: message provider message；"*" message
    ...
    TOOL_REGISTRY[name] = ToolEntry(instance=instance,
        providers=provs,
        risk=risk,
        enabled=enabled,)
...
def replace_instance(name: str, instance: BaseTool) -> None:
    """message providers / flags message,message(message)."""
...
def register_tool(*, name: str, providers: Iterable[str] | str = '*', enabled: bool = True):
    """
    message“message”message:import message.
    """
...
class ToolManager:
    ...
    @staticmethod
    def autodiscover(package: str = "argus.tools") -> None:
        """
        message.
        message register_instance(),message.
        """
    ...
    async def execute(self, tool_name: str,  tool_args: Dict[str, Any], tool: BaseTool, **kwargs) -> Tuple[bool, Optional[str | Dict]]:
        if self.hook_mgr:
            pre_ok, pre_msg, _ = await self.hook_mgr.emit(HookEvent.PreToolUse,
        ...
        if self.hook_mgr:
            post_ok, post_msg, _ = await self.hook_mgr.emit(HookEvent.PostToolUse,
                base_payload={"session_id": ""},
                tool_name=tool_name,
                tool_input=tool_args,
