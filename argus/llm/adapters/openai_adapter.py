...
class OpenAIAdapter():
    """
    message Responses API message Chat Completions API.
    ...
    # responses message async def _responses_stream_responses_async(self, messages, model, params) -> AsyncGenerator[ResponseEvent, None]:
        stream = await self._async.responses.create(model=model,
            input= messages,
            stream=True,
    ...
    #chat message async def _chat_stream_responses_async(self, messages, model, params) -> AsyncGenerator[ResponseEvent, None]:
        chat_msgs = _to_chat_messages(messages)
        stream = await self._async.chat.completions.create(model=model,
            messages=chat_msgs,
            stream=True,
            **{k: v for k, v in params.items() if k not in ("model", "api")})
        ...
        async for chunk in stream:
            ...
            for tc_delta in delta.tool_calls or []:
                idx = tc_delta.index
                data = tool_calls.setdefault(idx, 
            ...
            if finish_reason == "tool_calls":
                # tool_callmessagecall_id, name, arguments, type
            ...
            if finish_reason is not None:
                # messagetool_callsmessage, tool_callmessagecall_id, name, arguments, type
