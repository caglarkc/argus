...
from.prompt import (compression_prompt,
...
class MemoryMonitor:
    ...
    async def run_monitored(self, llm_client, cli, history: List[LLMMessage], tokens_used: int, turn: int) -> Tuple[int, str]:
        ...
        message,message.
        message:
            llm_client: LLM message.
            history: message.
            tokens_used: message token message.
            turn: message.
        message:
            Tuple[int, str]: message token message.
        """
        ...
        return await self._produce_valid_summary(llm_client, summary_text, history_text, tokens_used)

    async def _llm_ask_user_prompt(self, llm_client, prompt_text: str) -> Tuple[int, str]:
        """ 
        message LLM message.
        message: prompt_text - message: message token message
        """
        messages = [{"role": "user", "content": prompt_text}]
        model = self.cfg_mgr.get_active_model_name()
        buf: List[str] = []
        tokens_used: int = 0
        #TODO. message usage message
        ...
        try:
            async for event in llm_client.astream_response(messages=messages, api="chat", model=model):
                ...
                elif event.type in (LLM_Events.ERROR,
    ...
    async def _score_summary_quality(self, llm_client, summary_text: str, history_text: str, summary_tokens: int) -> Dict[str, Any]:
        """ 
        message(summary_text)message: summary_text - message
                  history_text - message
                  summary_tokens - message token message: message
        """
        required = [
        ...
        est_summary_tokens = max(1, len(summary_text) // 4) #TODO. message,messagellmmessage,llmmessage
        ratio_score = (est_summary_tokens / summary_tokens) if summary_tokens > 0 else 1.0
        
        fidelity = int(section_ratio * 100 * 0.3
    ...
    async def _produce_valid_summary(self, llm_client, summary_text: str, original_text: str, tokens_used: int) -> Tuple[int, str]:
        """ 
        message summary message,message“message”,
        message.
        """
        # message
        ...
        # message 1
        tokens_used, d1_text = await self._llm_ask_user_prompt(llm_client, first_downgrade_prompt.replace("<<SUMMARY>>",summary_text).replace("<<ORIGINAL>>", original_text))
        if d1_text:
        ...
        tokens_used, d2_text = await self._llm_ask_user_prompt(llm_client, second_downgrade_prompt.replace("<<SUMMARY>>",base).replace("<<ORIGINAL>>",original_text))
        if d2_text:
    ...
    def _fallback_compact_history(self, history: List[LLMMessage]) -> str:
        """ message:message 30%,message user message. """
        if not history:
            return ""
